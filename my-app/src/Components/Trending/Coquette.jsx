...
const Coquette = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/products")
      .then((res) => res.json())
      .then((data) => {
        const coquetteProducts = data.filter(
          (product) =>
            Array.isArray(product.trending)
              ? product.trending.includes("Coquette")
              : product.trending === "Coquette"
        );
        setProducts(coquetteProducts);
      })
  ...
  const handleFilterChange = (filterCategory, value, isChecked) => {
    setSelectedFilters((prev) => {
      ...
      if (isChecked) {
        ...
        updated[filterCategory] = updated[filterCategory].filter((v) => v !== value);
      }
      return updated;
    });
  };
  ...
  const applyFilters = () => {
    return products.filter((product) => {
      if (selectedFilters.Type.length > 0 && (!product.type || !product.type.some((t) => selectedFilters.Type.includes(t)))) {
        return false;
      }
      ...
      if (selectedFilters.Price.length > 0) {
        const matched = selectedFilters.Price.some((range) => {
  ...
  return (
    <div className="material-page">
      ...
      <h2 className="material-heading">Coquette Collection</h2>
      <p className="material-description">Discover our beautiful Coquette-themed products, crafted with elegance and charm.</p>
